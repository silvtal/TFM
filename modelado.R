...
option_list <- list(
  ...
  make_option(c("--tree"), type="character", default=paste0(home,"/99_otus_nodes.tree"),
              help="16S phylogenetic tree file name. The node names of the trees should be modified, and genuine names that match the ones of the --nodes file should be given for all. This could be done for example using R with the function 'makeNodeLabel' from 'ape' package. Default is './99_otus_nodes.tree' (Greengenes gg_13_5).", metavar="character"),
  make_option(c("--fasta"), type="character", default=paste0(home,"/99_otus.fasta"),
              help="16S sequences of all the leaves of the tree, in multifasta format. Default is './99_otus_nodes.tree' (Greengenes gg_13_5).", metavar="character"),
  ...
  make_option(c("--cores"), type="numeric", default=4,
              help="Number of cores to use in parallelization processes (mclapply and mcmapply). Default: 4.", metavar="numeric"))
